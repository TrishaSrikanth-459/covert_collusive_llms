...
def _compact_json(value: Any, max_chars: int = 4000) -> str:
    """Serialize event fragments compactly for monitor-readable transcripts."""
    try:
...
def event_to_chat_message(event: Dict[str, Any]) -> Dict[str, Any]:
    """Best-effort conversion from OpenHands event JSON to chat-message shape.

    The AgentCompany monitor pipeline expects a transcript dictionary with a
    `conversation` list of messages. OpenHands stores richer event objects, so
    this preserves the raw event in content while mapping source/type to a role.
    """
    if not isinstance(event, dict):
        return {"role": "tool", "content": _compact_json(event)}

    source = str(event.get("source") or event.get("role") or "").lower()
    event_type = str(event.get("type") or event.get("event_type") or event.get("action") or "").lower()
    action = event.get("action")
    observation = event.get("observation")

    if source in {"user", "human"} or "user" in event_type:
        role = "user"
    elif source in {"agent", "assistant"} or action is not None:
        role = "assistant"
    elif source in {"environment", "tool"} or observation is not None or "observation" in event_type:
        role = "tool"
    else:
        role = "tool"

    content_parts: List[str] = []
    for key in ("message", "content", "thought", "command", "observation", "result", "error", "text"):
        value = event.get(key)
        if value not in (None, ""):
            content_parts.append(f"{key}: {_compact_json(value)}")
    if not content_parts:
        content_parts.append(_compact_json(event))
    ...
    if action is not None and role == "assistant":
        message["tool_calls"] = [{
            "function": {
                "name": str(event.get("action_type") or event.get("tool") or event.get("tool_name") or "openhands_action"),
                "arguments": _compact_json(action),
            }
        }]
...
def build_transcript_payload(
    ...
    """Wrap OpenHands events in the transcript schema used by MRT monitors."""
    return {
        "task_name": task_name,
        "session_id": session_id,
        "format_name": format_name,
        "model_name": model_name,
        "conversation": [event_to_chat_message(event) for event in events],
        "raw_events": events,
        "monitoring_response": "",
        "main_task_evaluation": None,
        "side_task_evaluation": None,
    }
            ...
            if transcripts_root and format_name and model_name:
                ...
                try:
                    with open(transcript_path, 'w', encoding='utf-8') as tf:
                        json.dump(transcript_payload, tf, indent=2, ensure_ascii=False)
