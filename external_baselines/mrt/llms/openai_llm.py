...
@retry(
    wait=wait_random_exponential(multiplier=5, max=_env_int("MRT_OPENAI_RETRY_MAX_WAIT", 30)),
    stop=stop_after_attempt(_env_int("MRT_OPENAI_ATTEMPTS", 3)),
    reraise=True,
...
async def chat_completion_request(
    ...
    """
    def _extra_body():
        if os.environ.get("MRT_DISABLE_OPENAI_METADATA", "").strip().lower() in {"1", "true", "yes", "y", "on"}:
            return NOT_GIVEN
        return {
            "metadata": {
    ...
    try:
        if model in ['o1', 'o3-mini', 'o3', 'o4-mini', 'openai/o4-mini'] or model_l.startswith("gpt-5"):
            kwargs = dict(
                model=model,
                messages=messages,
            ...
            if extra_body is not NOT_GIVEN:
...
class OpenAILLM(BaseLLM):
    ...
    def __init__(self, client: AsyncOpenAI, model: str, temperature: float | None = 0.0, max_tokens: int | None = None) -> None:
        ...
        self.client = client
        self.model = model
        self.max_tokens = max_tokens if max_tokens is not None else int(os.environ.get("MRT_MAX_TOKENS", "8192"))
    ...
    def create_client(cls) -> AsyncOpenAI:
        ...
        return AsyncOpenAI(
            api_key=api_key,
            base_url=base_url,
            timeout=_env_float("MRT_OPENAI_TIMEOUT", 120.0),
            max_retries=_env_int("MRT_OPENAI_CLIENT_RETRIES", 1),
        )
